...
export class CommentRepository {
  static async findByPostId(postId, offset = 0, limit = 100) {
    ...
    const comments = rows.map((r) => new CommentEntity({ ...r, likeCount: r.like_count }));

    const commentMap = new Map();
    comments.forEach((c) => commentMap.set(c.id, { ...c.toJSON(), replies: [] }));
  ...
  static async create({ postId, authorId, content }) {
    const [result] = await pool.query(`INSERT INTO comments (post_id, author_id, content) VALUES (?, ?, ?)`, [postId, authorId, content]);
  ...
  static async getLikesByCommentId(commentId) {
    const [rows] = await pool.query(`SELECT * FROM comment_likes WHERE comment_id = ?`, [commentId]);
  ...
  static async create({ postId, authorId, content, parentId = null }) {
    const [result] = await pool.query(`INSERT INTO comments (post_id, author_id, content, parent_id) VALUES (?, ?, ?, ?)`, [postId, authorId, content, parentId]);
    return this.findById(result.insertId);
  }
