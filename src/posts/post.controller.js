...
// GET /api/posts?page=1&pageSize=10&sortBy=like_count&order=ASC
PostController.get("/", async (req, res) => {
  ...
  const posts = await PostService.getAllPosts(page, pageSize, sortBy, order);
  res.json(posts.map((p) => p.toJSON()));
});

// GET /api/posts/search?keyword=...&page=1&pageSize=10&sortBy=like_count&order=DESC
PostController.get("/search", async (req, res) => {
  try {
    const { keyword, page = 1, pageSize = 10, sortBy, order } = req.query;
    if (!keyword || keyword.trim() === "") {
      return res.status(400).json({ error: "Keyword is required" });
    }
    const posts = await PostService.searchPosts(keyword, parseInt(page), parseInt(pageSize), sortBy, order);
    res.json(posts.map((p) => p.toJSON()));
  } catch (err) {
...
// GET /api/posts/user/:id?sortBy=like_count&order=DESC
PostController.get("/user/:id", async (req, res) => {
  try {
    const sortBy = req.query.sortBy || "created_at"; // created_at, like_count, views
    const order = req.query.order || "DESC"; // ASC or DESC

    const posts = await PostService.getPostsByAuthorId(req.params.id, sortBy, order);
    res.json(posts.map((p) => p.toJSON()));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
...
PostController.post("/", authRequired, uploadTemp.array("images", 10), async (req, res) => {
  try {
    ...
    const postData = {
      ...
      categoryIds: Array.isArray(req.body.categoryIds) ? req.body.categoryIds : JSON.parse(req.body.categoryIds || "[]"),
    };
...
PostController.patch("/:id", authRequired, uploadTemp.array("images", 10), async (req, res) => {
  try {
    ...
    if (req.body.categoryIds) {
      updateData.categoryIds = Array.isArray(req.body.categoryIds) ? req.body.categoryIds : JSON.parse(req.body.categoryIds);
    }
...
PostController.delete("/:id/admin", adminRequired, async (req, res) => {
  try {
    await PostService.deletePost(req.params.id, req.user.id, true);
    res.json({ message: "Post deleted" });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
...
PostController.post("/:id/categories/add", authRequired, async (req, res) => {
  try {
    const categories = await PostService.addCategoriesToPost(req.params.id, req.user.id, req.body.categoryIds);
    res.json(categories.map((c) => c.toJSON()));
  } catch (err) {
    if (err.details) {
      return res.status(400).json({ error: err.details });
...
PostController.patch("/:id/categories", authRequired, async (req, res) => {
  try {
    const categories = await PostService.replaceCategoriesOfPost(req.params.id, req.user.id, req.body.categoryIds);
    res.json(categories.map((c) => c.toJSON()));
  } catch (err) {
    if (err.details) {
      return res.status(400).json({ error: err.details });
...
PostController.post("/:id/comments", authRequired, async (req, res) => {
  try {
    ...
    const comment = await CommentService.createComment(req.params.id, req.user.id, content, comment_id || null);
    const post = await PostService.getPostById(req.params.id);
