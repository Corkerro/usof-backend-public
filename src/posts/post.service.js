...
export class PostService {
  static async getAllPosts(page = 1, pageSize = 10, sortBy, order) {
    const offset = (page - 1) * pageSize;
    return PostRepository.findAll(offset, pageSize, sortBy, order);
  }
  ...
  static async getPostsByAuthorId(authorId, sortBy, order) {
    return PostRepository.findByAuthorId(authorId, sortBy, order);
  }
  ...
  static async deletePost(postId, authorId, isAdmin = false) {
    const post = await PostRepository.findById(postId);
    if (!post) throw new Error("Post not found");
  ...
  static async addCategoriesToPost(postId, authorId, categoryIds) {
    ...
    if (totalCategories > 10) {
      throw new Error(`Maximum 10 categories are allowed per post. Currently: ${existingIds.length}, adding: ${newCategoryIds.length}`);
    }
  ...
  static async searchPosts(keyword, page = 1, pageSize = 10, sortBy, order) {
    if (!keyword || keyword.trim() === "") return [];
    const offset = (page - 1) * pageSize;
    return PostRepository.search(keyword, offset, pageSize, sortBy, order);
  }
}
